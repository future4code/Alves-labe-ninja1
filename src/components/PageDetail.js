...
export default class PageDetail extends React.Component {
  ...
  PegarDetalhe = (id) => {
    axios
      .get(
        `https://labeninjas.herokuapp.com/jobs/${this.props.clickedCharacterId}`,
        {
          headers: {
      ...
      .then((response) => {
        ...
        console.log(this.props.clickedCharacterId);
      })
      .catch((error) => {
    ...
    const listaDetalhada = this.state.DetalhesDoServiço.map((servico) => {
      return (
        <div key={servico.id}>
          ...
          <p>Valor: R$ {servico.price.toFixed(2).replace(".", ",")}</p>
          <p>Forma de pagamento: {servico.paymentMethods}</p>
          <button onClick={this.props.goToPageCards}>
            Voltar para lista
          </button>
        </div>
      );
    });
    return <div>{listaDetalhada}</div>;
  }
}
